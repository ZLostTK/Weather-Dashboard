<template>
    <div class="glass-card p-4 sm:p-6 animate-slide-up">
        ...
        <div class="h-48 sm:h-64 w-full relative">
            ...
            <div
                v-if="loading"
                class="absolute inset-0 flex items-center justify-center bg-card/50 backdrop-blur-sm rounded-lg"
            >
                <div class="text-muted-foreground text-sm">Cargando datos del gráfico...</div>
            </div>
            <div
                v-else-if="!forecast && !props.loading"
...
const generateFallbackData = () => {
    if (selectedPeriod.value === 'hourly') {
        ...
        const feelsLike = temperatures.map(
            temp => Math.round((temp + Math.random() * 3 - 1.5) * 10) / 10
        );
        ...
        const temperatures = Array.from(
            { length: 7 },
            () => Math.round((10 + Math.random() * 15) * 10) / 10
        );
        const feelsLike = temperatures.map(
            temp => Math.round((temp + Math.random() * 2 - 1) * 10) / 10
        );
...
const createChart = async () => {
    ...
    try {
        ...
        try {
            ...
            console.error('❌ Canvas context is not functioning properly:', contextError);
            return;
        }
    } catch (error) {
        ...
        if (props.forecast && props.forecast.forecast && props.forecast.forecast.forecastday) {
            if (selectedPeriod.value === 'hourly') {
                ...
                if (days && days.length > 0) {
                    ...
                    feelsLike = days.map(
                        day => Math.round(((day.day.maxtemp_c + day.day.mintemp_c) / 2) * 10) / 10
                    );
                }
            }
        }
    ...
    try {
        ...
        if (!ctx || !chartRef.value || !chartRef.value.isConnected) {
            console.error('❌ Context or canvas invalid at chart creation time');
            return;
        }
