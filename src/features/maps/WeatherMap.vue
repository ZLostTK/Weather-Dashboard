<template>
  <div class="glass-card p-6 animate-fade-in">
    <div class="flex items-center justify-between mb-6">
      <h3 class="text-xl font-semibold text-white">Interactive Weather Map</h3>
    ...
    <div class="relative h-96 bg-white/5 rounded-lg overflow-hidden">
      ...
      >
        <div class="relative">
          <div class="w-8 h-8 bg-white/80 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/40 shadow-lg">
            <span class="text-xs font-semibold text-gray-800">
              {{ Math.round(marker.temp) }}°
            </span>
          </div>
          <div class="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-white/80 rotate-45 border-r border-b border-white/40"></div>
        </div>
      </div>
      ...
      <div
        v-if="selectedMarker"
        class="absolute bottom-4 left-4 right-4 bg-black/60 backdrop-blur-md rounded-lg p-4 text-white"
      >
        <div class="flex items-center justify-between">
          <div>
            <h4 class="font-semibold">{{ selectedMarker.city }}</h4>
            <p class="text-sm text-white/70">{{ selectedMarker.condition }}</p>
          </div>
          <div class="text-right">
            <div class="text-xl font-bold">{{ Math.round(selectedMarker.temp) }}°C</div>
            <div class="text-sm text-white/70">Feels {{ Math.round(selectedMarker.feelsLike) }}°</div>
          </div>
          <button
            @click="selectedMarker = null"
            class="ml-4 p-1 hover:bg-white/10 rounded"
          >
            <X class="w-4 h-4" />
          </button>
        </div>
...
const getLayerDescription = () => {
  switch (selectedLayer.value) {
    case 'temp': return 'Temperature overlay with regional variations and heat maps';
    case 'clouds': return 'Cloud coverage, density patterns and satellite imagery';
    case 'precipitation': return 'Rainfall intensity, precipitation radar and forecasts';
    case 'wind': return 'Wind patterns, speed variations and directional flows';
    default: return 'Current weather conditions across regions';
  }
};
