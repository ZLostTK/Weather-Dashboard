...
const stats = computed(() => {
  ...
  const avgHumidity = Math.round(days.reduce((sum, day) => sum + day.day.avghumidity, 0) / days.length);
...
const createMainChart = () => {
  ...
  mainChart.value = new ChartJS(ctx, {
    type: activeChart.value === 'precipitation' || activeChart.value === 'wind' ? 'bar' : 'line',
